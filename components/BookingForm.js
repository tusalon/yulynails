...
// ✅ CORREGIDO: Eliminado código JSX mal ubicado
...
function BookingForm({ service, profesional, date, time, onSubmit, onCancel, cliente }) {
    ...
    function generarArchivoCalendario(bookingData, nombreNegocio) {
        ...
        const linea6 = `WhatsApp: ${bookingData.cliente_whatsapp}`;
        const linea7 = ``;
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="bg-pink-50 p-3 rounded-lg border border-pink-200">
                            <p className="text-sm text-pink-700">
                                <span className="font-semibold">Tus datos:</span> {cliente.nombre} - {window.formatearNumeroParaUI ? window.formatearNumeroParaUI(cliente.whatsapp, window.getNegocioIdActual()) : `+${cliente.whatsapp}`}
                            </p>
                        </div>
