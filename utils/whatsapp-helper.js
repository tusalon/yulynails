...
// ✅ CORREGIDO: variable telefonoLimpio definida correctamente
...
window.enviarWhatsApp = function(telefono, mensaje) {
    try {
        ...
        // Obtener el código de país del negocio actual
        const negocioId = window.getNegocioIdActual();
        ...
        if (!numeroCompleto.startsWith(codigoPais)) {
            numeroCompleto = codigoPais + telefonoLimpio;
        }
...
📱 *Enviar comprobante a este WhatsApp:* 
   ${window.getFormatoPais(window.getNegocioIdActual())} ${configNegocio.telefono || '00000000'}
